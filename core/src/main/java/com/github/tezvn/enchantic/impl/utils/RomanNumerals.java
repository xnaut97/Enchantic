...
/**
 * A class can be used to convert normal numbers to roman ones and otherwise.
 */
public class RomanNumerals {
	 ...
	 *
	 * @param romanNumeral
	 *            Roman Numeral string to parse
	 * @return integer value (0 if invalid string)
	 */
	public static int fromRomanNumeral(final String romanNumeral) {
	...
	private static final Map<Character, Integer> CHAR_TO_NUMBER = ImmutableMap.copyOf(Arrays.stream(RomanNumber.values())
			.filter(num -> num.name().length() == 1)
			.collect(Collectors.toMap(num -> num.name().charAt(0), RomanNumber::getValue)));
}